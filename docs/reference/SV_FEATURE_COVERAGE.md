...
| slice write to an array CELL `arr[k][hi:lo] = v` (clocked, comb, continuous) | ✅ | a read-modify-write of the cell; in a comb block the slice folds into the earlier whole write of the cell (the value so far), a guarded or first slice write refused by name; continuous slices of a cell assembled at the cell's width (`array_cell_*` sweep rows, Icarus, both modes; F56) |
| constant-bank genvar target `assign y[c][w] = ..`, `y[c][w+k]` | ✅ | a lane-rolled memory write with the bank pinned and the offset in the head; the windows of a combinational memory must TILE it (certified at the epilogue); `y[c][2*w]` stays a named refusal (`constant_bank_genvar_window`; F58) |
| clock-gate enable as an expression or a constant | ✅ | hoisted into a named gate bit; `.en(1'b1)` is the base clock, `.en(1'b0)` a never-ticking domain (warned) (`clock_gate_*_enable` rows; F57) |
| range of a PACKED 2-D element `o[1][7:0] = ..` (continuous) | ✅ | the flat offset `idx*W + lo`, partial elements tie the gap to 0 (`packed_element_range_assign`; F60); the CLOCKED form and a slice of a lane-rolled root are refused by name (`packed_element_range_clocked`) |
| primitive output on a select `.o(y[1])`, `.o(y[3:0])` | ✅ | hoisted to a temp and distributed to the element/range at both instance sites (`test_width_aware_primitive_category`; F60) |
| nested lane register `for(i) for(j) r[i][j] <= ..` with an async reset | ✅ | the force and release rules bind every lane variable by its packed extent, as the write rule does (`nested_lane_reg_async_reset`; F61) |
| a RUN of whole lanes written by one slice `ck[23:0] = {24{v}}`, `ck[31:24] = '0` | ✅ | a partial lane loop when every lane gets the same value -- a replicated unit or a per-lane-uniform constant (`lane_run_slice_write`; F62); any other slice of a lane signal stays refused by name |
| constant bank + genvar on a PACKED 2-D `way[1][w] = dec[0][w+6] \| ..` | ✅ | the flat lane `gv + (bank*W + c)`, read and written, either outer range direction (`packed_bank_genvar_select`; F63 -- the packed twin of F58) |
| affine window of a wide word into a lane target `rd[i*2 +: 2] = {big[a*i+b -: 2], ..}` | ✅ | kept on the word path (the shift by the affine amount, masked); the value TRUNCATED to the lane width as an assignment does (`affine_window_concat_lane`, random stimulus; F64, F65) |
| ternary filling a lane element `en[b][1:0] = cen[b] ? .. : ..`; a lane-bit selector with a generate-local in the arms | ✅ | the element field is the value; a lane target's per-bit conditional reads its selector PER LANE (`lane_field_ternary`, `lane_ternary_selector_local`; F65) |
| black box (`sources.json` `blackbox`): a module not translated, outputs ANY value | ✅ | every output `dontcare_at` at every instant, the choice in the power-on companion, both modes; with no definition in scope the outputs are listed and widths come from the parent's connections; wide outputs get guidance and are pinned in the scenario (`test_blackbox_outputs_are_unconstrained`, `test_blackbox_refusals_and_guidance`) |
| assigned `x` in MODULAR mode | ✅ | the declaration lifted per instance and the choice emitted per instance path -- flat-only before 2026-09-08, silently (`test_x_dontcare_is_a_real_choice_modular`) |
| lane signal written by WORD slices (`dec[1] = ..` on a net read per bit; `ck[23:0] = ..` on a lane-rolled vector) | ✅ | lanes DERIVED from the slices: a run inside one part reads it at `I*ew - off`, a straddling lane from constant pieces, gaps tied to 0, overlaps refused (`lane_elements_from_word_writes`, `lane_slice_straddles_lanes`; F67) |
| ternary on a one-bit lane with a boolean arm `sel ? (a[w] \| b[w]) : (bk == w)` | ✅ | the boolean arm hoisted into a lane-local bit at the lane write site (`lane_ternary_compare_genvar`; F68) |
| library cells INSIDE a generate with per-iteration pins (`AN2 (.Z(y[i]))`, `FF (.q(q[i]), .d(d[i]))`) | ✅ | the lane rule / lane register the equivalent `assign` / `always_ff` gets; a ROW flop of a packed 2-D net lifted into one-bit lane flops `q(I, J) <= x(I, J)`; a row read of a per-bit-laned net assembled from its bits (`primitive_cells_in_generate`, `row_flop_lifted_per_bit`; F69) |
| a temporary hoisted in a NESTED generate reading both indices | ✅ | a lane over every index it mentions, extents from the loop nest (`nested_generate_hoisted_arm`; F70); a partial or strided loop in the nest refused by name |
| genvar in a ternary's TEST; a single-iteration generate's genvar | ✅ | seen by the walker (a lane write, not a broadcast); a one-iteration loop's genvar is its constant everywhere (`single_iteration_generate_compare`; F71) |
| `--coverage FILE` | ✅ | writes the line report (every source line with its status), the stderr summary unchanged |
| width-aware primitive category `comb_w` | ✅ | a plugin builder that receives every pin's width at the call site -- decoders, encoders, find-first over a parameterized width (`test_width_aware_primitive_category`; F59); not inside an instance array (refused by name) |
